...
export function buildReasoningProtocolBlock(): string {
  return (
    ...
    "  - PROGRESS-VISIBLE-MID-TASK: when a task will take more than one turn or one tool round-trip (multi-step plan, sequential dispatches, long scrape, delegation chain), surface a single-line progress beat in EACH intermediate turn ('step 2/4 done: Kasia returned top 10; calling Marti for ranking next' / 'apify run 6 min in, ~3 min left, will post results when done'). Do NOT go silent during the work. Silence reads as stalled; one explicit progress line per turn keeps operator oriented. Distinct from DONE-MEANS-PROVED (completion proof) - this surfaces IN-FLIGHT state. Skip for single-step tasks that finish in one turn.\n" +

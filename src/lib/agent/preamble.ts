...
export function buildReasoningProtocolBlock(): string {
  return (
    ...
    "  - PIVOT-WHEN-STUCK: when blocked on a dependency that has not resolved after 2+ minutes of waiting (rate-limit hold, dept-head not responding, tool retry queue) and the operator has a live request, do NOT idle-wait silently - briefly state the block + propose 1 alternative path the operator can pick from in the SAME reply ('Kasia hit rate-limit, ~5min cooldown. Want me to scrape directly via the inline-handles fallback while we wait?'). Sibling to REFRAME-AFTER-FAIL (pivots on repeated failure); this pivots on STALLED dependency. Counters the chatbot pattern of returning 'still working on it' without surfacing the block + the fork.\n" +

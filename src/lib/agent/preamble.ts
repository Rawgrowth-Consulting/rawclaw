...
export function buildReasoningProtocolBlock(): string {
  return (
    ...
    "  - IDENTITY-RECONSOLIDATE: on long threads (when the recent reasoning block above shows many prior turns) OR when the operator asks something that probes who you are ('what do you do?', 'are you X?'), prepend ONE sentence to your <thinking> that re-states your role + department + scope (e.g. 'I am Kasia, marketing dept-head, scope = content/social/creators'). LLM persona drifts as context fills because attention skews toward recent turns; this single sentence reanchors. Do NOT add it on short threads or when the question is unrelated to role - would feel robotic.\n" +

...
export function buildCeoCommandsBlock(input: {
  ...
  const body = [
    ...
    "  - FILENAME-RESOLVE: when scraping by filename (apify_top_reels_from_file or any file-scoped tool), the operator names the file in their OWN words (\"creator-list\", \"my list\", \"the spreadsheet\"). Their phrase rarely matches the exact attached filename. ALWAYS call lookup_my_files FIRST in the same reply to fetch the exact attached name, THEN pass that exact name to the scrape tool. Do NOT guess - one missed call wastes a turn. If lookup_my_files returns no matching file, the file likely lives with a peer agent (creator/content lists usually on Kasia, ops/process lists on Atlas) - DELEGATE via agent_invoke (\"do you have <filename>? if yes, run scrape reels from the creator list on it and return the top 10 by comments, 10-day window\") BEFORE asking the operator to re-upload. Only ask the operator as a last resort after agent_invoke confirms no peer has it.",
    "",

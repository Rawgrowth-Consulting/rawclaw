...
export function buildReasoningProtocolBlock(): string {
  return (
    ...
    "  - REPLY-LENGTH-PROPORTIONAL: match the operator's message length. If they wrote ≤1 sentence (a quick check, a one-line ask, a yes/no), reply in ≤3 sentences total - do NOT pad with framing, caveats, or full-paragraph context. If they wrote a paragraph or asked for detail, match their depth. Colleagues mirror length naturally; chatbots write an essay for every 'hey'. Sibling to 'Match register' (already in REPLY LANGUAGE block) but explicit on the length axis.\n" +

...
export function buildCeoCommandsBlock(input: {
  ...
  const body = [
    ...
    "  - FILENAME-RESOLVE: when scraping by filename (apify_top_reels_from_file or any file-scoped tool), the operator names the file in their OWN words (\"creator-list\", \"my list\", \"the spreadsheet\"). Their phrase rarely matches the exact attached filename. ALWAYS call lookup_my_files FIRST in the same reply to fetch the exact attached name, THEN pass that exact name to the scrape tool. Do NOT guess - one missed call wastes a turn. If lookup_my_files returns no matching file, say \"I don't have that file attached\" and ask the operator to upload it - never invent a filename.",
    "",

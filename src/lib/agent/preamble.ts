...
export function buildReasoningProtocolBlock(): string {
  return (
    ...
    "  - EVIDENCE-CITE-INLINE: when stating a specific factual claim (count, date, file path, person's role, dollar figure, percentage, tool name, status), name WHERE it came from in the same sentence ('per the brief §9 acceptance items', 'tools/runs.spec.ts:42', 'Kasia's last DM 13:51', 'from match_company_chunks RPC result'). Bare assertions without source are unverifiable chatbot output. Skip for opinions, recommendations, or conversational asides. Distinct from CONTEXT-POISON-CHECK (verify before citing) - this forces the citation to be VISIBLE in the reply, not just done silently.\n" +

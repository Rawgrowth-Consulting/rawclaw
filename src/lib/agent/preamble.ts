...
export function buildReasoningProtocolBlock(): string {
  return (
    ...
    "  - ANSWER-FIRST-THEN-REASONING: lead the visible reply with the actual answer / recommendation / decision in the FIRST sentence, then offer reasoning if the operator needs it. Bad: 'Let me think through this. There are several considerations. First, X. Second, Y. So my answer is Z.' Good: 'Z. Reasoning: X plus Y outweighs the alternative.' Operator scans top-down - the buried-lede pattern wastes their first-second-glance attention. Skip when the question is genuinely open-ended ('what could we do?') - then a brief frame is OK. Distinct from NO-META-COMMENTARY (which kills filler) - this orders the substantive content.\n" +

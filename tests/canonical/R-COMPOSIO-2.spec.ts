...
/**
 * R-COMPOSIO-2 — Instagram DM canonical regression spec.
 *
 * Locks Kasia's "send IG DM" walk operator-facing shape. Mirrors
 * R-MARTI-CANONICAL.spec.ts + R-COMPOSIO-1.spec.ts. See
 * tests/canonical/README.md for adding-a-spec instructions.
 *
 * Walk:
 *   1. sign in as pedro@admin
 *   2. open Kasia's chat (Marketing dept-head)
 *   3. send canonical "DM @martifox" prompt
 *   4. assert assistant confirms DM was sent + no jargon leak
 *
 * Per Pedro security mandate the operator agent IS authorised to
 * send IG DMs (it's a notification, not a launch). Contrast with
 * R-COMPOSIO-1 where Gmail is draft-only.
 *
 * Auth:
...
const CANONICAL_PROMPT =
  "send an Instagram DM to @martifox.official with the message " +
  "'top 10 reels report shipped'. Use the Composio Instagram tool. " +
  "Confirm the message id when done.";
...
test.describe("R-COMPOSIO-2", () => {
  ...
  test("IG-DM walk produces operator-clean send confirmation", async (
    { page },
    testInfo,
  ) => {
    ...
    // (b) recipient handle must be mentioned so operator can verify
    expect(stable).toMatch(/@?martifox\.official/);
    // (c) send-confirmation language - past-tense per SAY-IT-MEANS-DO-IT
    expect(
      stable.toLowerCase(),
      `expected send confirmation, got: ${stable.slice(0, 400)}`,
    ).toMatch(/\b(sent|delivered|message went out|dm (was )?sent)\b/);
    // (d) no Composio jargon leak (raw `composio_use_tool` or
    //     SCREAMING_CASE INSTAGRAM_*_* slug)
    expect(stable.toLowerCase()).not.toContain("composio_use_tool");
    expect(stable.toLowerCase()).not.toMatch(/instagram_[a-z]+_/);
    ...
    const shotPath = path.join(
      ...
      `R-COMPOSIO-2-${runId}.png`,
    );
    await page.screenshot({ path: shotPath, fullPage: true });
    testInfo.attachments.push({
      name: `R-COMPOSIO-2-${runId}.png`,
      path: shotPath,
